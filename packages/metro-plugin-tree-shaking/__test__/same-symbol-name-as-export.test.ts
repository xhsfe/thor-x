...
test('import export specifier from lib', () => {
  const METRO_TREE_SHAKING_CACHE_ROOT = path.join(
    __dirname,
    '../.optimize/same-symbol-name-as-export',
  )
  const OUTPUT_ROOT = path.join(
    __dirname,
    '../dist/same-symbol-name-as-export',
  )
  const ENTRY_FILE = './__test__/same-symbol-name-as-export.ts'
  const BUNDLE_OUTPUT = './dist/same-symbol-name-as-export'
