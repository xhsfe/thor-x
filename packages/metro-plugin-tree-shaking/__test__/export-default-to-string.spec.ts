...
test('use import specifier to rename lib\'s named export', () => {
  const METRO_TREE_SHAKING_CACHE_ROOT = path.join(
    __dirname,
    '../.optimize/export-default-to-string',
  )
  const OUTPUT_ROOT = path.join(
    __dirname,
    '../dist/export-default-to-string',
  )
  const ENTRY_FILE = './__test__/export-default-to-string.ts'
  const BUNDLE_OUTPUT = './dist/export-default-to-string'
