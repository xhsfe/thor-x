...
test('import export specifier from lib', () => {
  const METRO_TREE_SHAKING_CACHE_ROOT = path.join(
    __dirname,
    '../.optimize/side-effect-variable-declaration',
  )
  const OUTPUT_ROOT = path.join(
    __dirname,
    '../dist/side-effect-variable-declaration',
  )
  const ENTRY_FILE = './__test__/side-effect-variable-declaration.ts'
  const BUNDLE_OUTPUT = './dist/side-effect-variable-declaration'
