...
test('use import specifier to rename lib\'s named export', () => {
  const METRO_TREE_SHAKING_CACHE_ROOT = path.join(
    __dirname,
    '../.optimize/import-reexport-star-export',
  )
  const OUTPUT_ROOT = path.join(
    __dirname,
    '../dist/import-reexport-star-export',
  )
  const ENTRY_FILE = './__test__/import-reexport-star-export.ts'
  const BUNDLE_OUTPUT = './dist/import-reexport-star-export'
