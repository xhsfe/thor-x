...
test('use import specifier to rename lib\'s named export', () => {
  const METRO_TREE_SHAKING_CACHE_ROOT = path.join(
    __dirname,
    '../.optimize/import-reexport-destruct-name',
  )
  const OUTPUT_ROOT = path.join(
    __dirname,
    '../dist/import-reexport-destruct-name',
  )
  const ENTRY_FILE = './__test__/import-reexport-destruct-name.ts'
  const BUNDLE_OUTPUT = './dist/import-reexport-destruct-name'
