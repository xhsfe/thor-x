...
test('import export specifier from lib', () => {
  const METRO_TREE_SHAKING_CACHE_ROOT = path.join(
    __dirname,
    '../.optimize/named-export-with-side-effect',
  )
  const OUTPUT_ROOT = path.join(
    __dirname,
    '../dist/named-export-with-side-effect',
  )
  const ENTRY_FILE = './__test__/named-export-with-side-effect.ts'
  const BUNDLE_OUTPUT = './dist/named-export-with-side-effect'
