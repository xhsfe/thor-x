...
test('import export specifier from lib', () => {
  const METRO_TREE_SHAKING_CACHE_ROOT = path.join(
    __dirname,
    '../.optimize/import-cjs-reexport-empty',
  )
  const OUTPUT_ROOT = path.join(
    __dirname,
    '../dist/import-cjs-reexport-empty',
  )
  const ENTRY_FILE = './__test__/import-cjs-reexport-empty.ts'
  const BUNDLE_OUTPUT = './dist/import-cjs-reexport-empty'
