...
test('use import specifier to rename lib\'s named export', () => {
  const METRO_TREE_SHAKING_CACHE_ROOT = path.join(
    __dirname,
    '../.optimize/import-reexport-star-name',
  )
  const OUTPUT_ROOT = path.join(
    __dirname,
    '../dist/import-reexport-star-name',
  )
  const ENTRY_FILE = './__test__/import-reexport-star-name.ts'
  const BUNDLE_OUTPUT = './dist/import-reexport-star-name'
